...
grep "<unk>" $lang_out/phones.txt | cut -d' ' -f2 | while read p; do echo $p ; grep -v " $p " $lang_out/ali.phones > $lang_out/ali.phones.tmp; mv $lang_out/ali.phones.tmp $lang_out/ali.phones ; done

cat $lang_out/ali.phones |  fstcompile --acceptor=true | fstarcsort --sort_type=ilabel > $lang_out/G.fst
