...
test_set=                         # becomes exp/nnet3_cleaned or whatever.
...
if [ $stage -le 1 ] && [ ! -d data/${train_set}_sp ]; then
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 100 --cmd "$train_cmd" \
    data/${train_set}_sp data/${train_set}/lang $gmm_dir $ali_dir || exit 1
fi
